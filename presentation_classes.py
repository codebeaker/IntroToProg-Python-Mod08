...
class IO:
    ...
    @staticmethod
    def output_employee_data(saved_data: list, unsaved_data: list):
        """ This function displays employee data to the user

        ChangeLog: (Who, When, What)
        RRoot,1.1.2030,Created function

        :param saved_data: list of employee objects that have already been saved
        :param unsaved_data: list of employee objects that have not been saved
        ...
        if unsaved_data != []:
            print("--!--" * 10)
            print("This data has not yet been saved:")
            for employee in unsaved_data:
                if employee.review_rating == 5:
                    message = " {} {} is rated as 5 (Leading)"
                elif employee.review_rating == 4:
                    message = " {} {} is rated as 4 (Strong)"
    ...
    def move_unsaved_to_saved(unsaved_data: list, saved_data: list):
        """
        This function is to move unsaved data to saved data and clear
        the unsaved list
        :param unsaved_data: a list that holds unsaved employee objects
        :param saved_data: a list that holds saved employee objects
        :return:
        """
        for employee in unsaved_data:
            saved_data.append(employee)
        unsaved_data.clear()
